...
object Layout {
    ...
    fun place(view: View, x: Int, y: Int, width: Int, height: Int): List<Node> {
        val out = mutableListOf<Node>()
        arrange(view, x, y, width, height, out)
        return out
    }

    /** Lays a page out at its own size, centred on the canvas. */
    fun centred(view: View, canvasWidth: Int, canvasHeight: Int): List<Node> {
    ...
    private fun arrange(view: View, x: Int, y: Int, width: Int, height: Int, out: MutableList<Node>) {
        when (view) {
            ...
            is Panel -> {
                ...
                arrangeChildren(view, innerX, innerY, innerWidth, innerHeight, out)
            }
        }
    }
    ...
    private fun arrangeChildren(
        ...
        panel.children.forEachIndexed { index, child ->
            ...
            if (row) {
                arrange(child, x + cursor, y + crossOffset, alongSize, crossSize, out)
            } else {
                arrange(child, x + crossOffset, y + cursor, crossSize, alongSize, out)
            }
