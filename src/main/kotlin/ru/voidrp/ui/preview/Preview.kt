...
/**
 * Draws a page to a PNG, the way the client would — without a client.
 *
 * Designing an interface by asking someone to log in, open it and send a screenshot is a
 * slow way to move a button four pixels. Everything needed to draw the page is already on
 * this side: the layout produces exact geometry and the font sheets are the ones the
 ...
 * that survive the trip, opacity to its sixteen steps, and what the client would throw
 * away is thrown away here too — what shows up in the picture is what shows up in the game.
 *
 * ```kotlin
 * Preview.render(MyPage(), File("my-page.png"))                        // 16:9
 * Preview.render(MyPage(), File("narrow.png"), Viewport.parse("4:3")!!)
 * ```
 *
 * Item pictures are the one thing the server does not have — they live in the client and
 * the pack never carries them. Point `VOIDRP_CLIENT_JAR` at a client jar to have them
 * drawn; without it an item is an empty slot.
 */
object Preview {
